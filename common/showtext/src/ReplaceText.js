...
class ReplaceText extends XttJS {
	...
	static getWeightedRandom(randomList, weightedList) {
		return super.getWeightedRandom(randomList, weightedList.map(v => super.getTextNum(v)));
	}
	static nonrandom(min = 1, max = 10, variable) {
		if ((min = super.getTextNum(min)) === (max = super.getTextNum(max))) { return max; } // 如果两值相等则返回
		if (min > max) {
			let temp = min;
			min = max;
			max = temp;
	...
	static getVariable(key) {
		...
		if (res === undefined) {throw '没有' + key +'变量哦';}
		if (res.next) {
			let data = res.next();
			if (data.done) {
				return "已经获取完了哦~"
